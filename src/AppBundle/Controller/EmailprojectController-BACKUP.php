...
class EmailprojectController extends Controller
{
    ...
    public function newAction(Request $request)
    {
        ...
        $loggedInUser = $this->getUser();
        //$username = $loggedInUser->getUsername();
        $emailproject->setUsername($loggedInUser->getUsername())->setEmail($loggedInUser->getEmail());
    ...
    public function buildAction(Request $request, Emailproject $emailproject)
    {
       ...
       // $finder = new Finder();
       // this wasn't good $finder->in($_SERVER['DOCUMENT_ROOT'].'\app\Resources\views\company\company_1')->files()->name('_templates.json');
        
      //$finder->in($this->get('kernel')->getRootDir().'\Resources\views\company\company_1')->files()->name('_templates.json');
     ...
     // var_dump($moduleNames);
      //echo $this->get('kernel')->getRootDir().'/Resources/views/company/sdta/_templates.json';

        //foreach($finder as $file){
          // Dump the absolute path
          //var_dump(finder->getRealPath());
    ...
    public function ajaxAction(Request $request)
    {
      ...
      //echo $loggedInUser['company'];
      //$company = $emailproject->getCompany();
      //need to find another way to get the company, maybe from logged in user
      //the render below doesn't work if any includes in template files can't be found
      //so need to fix
       switch($options['flag']){
                ...
                case "dropModule":
                  ...
                  return $this->render('company/'.$company.'/templates/'.$options['module'].'.lyt.php', 
                             array('module' => $options['module'])         
                          ); 
                  break;
                case "saveProject":
                  $html = $options['html'];
        ...
        //$emailprojects = $em->getRepository('AppBundle:Emailproject')->findAll();
           ...
           //$d = new \DateTime();
           //$emailproject = new Emailproject("wtf", "title",$d);
   
          // var_dump($emailproject);
           ...
           //DON'T DELETE, YOU NEED THIS BECAUSE SAVE DOESN'T RE
           return new Response($options['module']);
       ...
       // $editForm->handleRequest($request);
          ...
          // return $this->redirectToRoute('emailproject_edit', array('id' => $projectID));
      ...
      //return $this->redirectToRoute('product_overview');
          ...
          //return $this->render('company/'.$company.'/templates/divider.lyt.php'); 
